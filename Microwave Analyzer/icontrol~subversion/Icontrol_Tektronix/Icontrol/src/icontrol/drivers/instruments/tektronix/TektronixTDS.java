...
// TODO 2* add instrument to list of available instruments in javadoc, add manual
 ...
 *
 */
package icontrol.drivers.instruments.tektronix;
 ...
 * Tektronix TDS Series oscilloscope. It was developed and tested on
 * a Tektronix TDS 774-A oscilloscope, and should also work for the models
 * TDS 410A, 420A, 460A, 520A, 524A, 540A, 544A, 620A, 640A, 644A, 684A,
 * 744A & 784A.<p>
 *
 * All device commands that the TektronixTDS understands are
 * implemented here.<p>
 *
 * <h3>Instrument Commands:</h3>
 * <ul>
 *  <li>{@link #HardCopy(String) }
 * </ul>
 *
...
@iC_Annotation(CommPorts=CommPorts.GPIB,
                InstrumentClassName="Tektronix TDS")
public class TektronixTDS extends Device {
    ...
    private static final Logger m_Logger = Logger.getLogger("iC.Instruments.TektronixTDS");
    ...
    // @Karl: You can disregard this method for the beginning
    /**
     ...
     */
    // TODO 3* remove method and add appropriate response to *IDN query into iC.properties
    @Override
    public String getInstrumentNameIdentifier() {
        return "";
    }


    /**
     * Overwritten <code>Device.Open</code> method for additional initializations
     * after establishing the connection to the Instrument. Additional to what's
     * done in <code>Device.Open</code> this method does ....
     *
    ...
    public void Open() throws IOException, ScriptException {
        ...
        // @Karl: THESE ARE JUST EXAMPLES THAT I HAVE COPIED FROM YOKOGAWA SCOPE
    ...
    /**
     * Saves screenshot of the scope to a file.<p>
     *
     * @param FileName The FileName for the screen shot. Should include the 
     * proper file extension.
     * 
     * @throws ScriptException bubbles up from <code>QueryCommand</code>.
     */
    // <editor-fold defaultstate="collapsed" desc="Hardcopy">
    @AutoGUIAnnotation(
        DescriptionForUser = "<html>Saves a screenshot of the scope in a file.</html>",
        ParameterNames = {"File Name"},
        DefaultValues = {"Hardcopy.bmp"},
        ToolTips = {"Should include the File Extension"})
    @iC_Annotation(  MethodChecksSyntax = false )
    public void HardCopy(String FileName) 
        ...
        // retreive the screen shot
        String ScreenShot = QueryCommand("HARDCopy STARt");
        
        
        byte[] ScreenShotBinary;
        try {
            
            // convert answer to byte[]
            ScreenShotBinary = ScreenShot.getBytes(CHARACTER_ENCODING);
        
        } catch (UnsupportedEncodingException ex) {
            String str = "The character encoding " + CHARACTER_ENCODING + 
                    "is not supported. Please choose a different encoding.\n";
        ...
        try {
            ...
            String str = "An error occured during writing the Screen Shot data to a file:"
                    + ex.getMessage() + "\n";
            throw new ScriptException(str);
        }
...
//        }
    ...
    /**
     * Changes the measurement type of the specified channel. This method 
     * performs a Syntax-Check.
     *
     * @param Channel The channel number to change the measurement type
     * @param Type The 
     *
     * @throws IOException re-thrown from <code>QueryInstrument</code>
     * @throws DataFormatException when the Syntax Check failed.
     */
    // <editor-fold defaultstate="collapsed" desc="MeasurementType">
    @AutoGUIAnnotation(
        DescriptionForUser = "<html>Set the Measurement Type.</html>",
        ParameterNames = {"Channel# {[1,4]}", "Measurement Type"},
        DefaultValues = {"1", "Amplitude"},
        ToolTips = {"", "<html>Can be AMPLITUDE, AREA, BURST...</html>"})
    @iC_Annotation(  MethodChecksSyntax = true )
    public void MeasurementType(int Channel, String Type)
        ...
        // @KArl: You will probably check the Channel number more often, so 
        // I suggest to make a CheckChannelNumber method (see e.g. LakeshoreTC)
        // Syntax check the channel number
        if (Channel < 1 || Channel > 4) {
            String str = "The Channel Number must be between 1 and 4.\n";

            throw new DataFormatException(str);
        }

        // Syntax check the measurement type
        final List<String> MEASUREMENT_TYPES =
                Arrays.asList("AMPLITUDE", "AREA", "BURST");   // the entries need to be in uppercase letters!
        if ( !MEASUREMENT_TYPES.contains(Type.toUpperCase()) ) {
            String str = "The Measurement Type '" + Type + "' is not valid.";
            str += "Please select a value from:\n " + MEASUREMENT_TYPES.toString() + ".\n";
            throw new DataFormatException(str);
        }
        ...
        String str = String.format(Locale.US, "MEASUREMENT:MEAS%d:TYPE %s", Channel, Type);
