...
public class HP4192 extends Device {
     ...
     *
     * @throws IOException When GPIB communication caused an error
     */
    ...
    private void ClearQueue()
        ...
        if (m_GPIB_Driver != null) {
            ...
            // show to the user
            //icontrol.IcontrolView.DisplayStatusMessage(str);
     ...
     * with <code>ret.Valid=false</code> if the received answer was not as expected. The
     * returned value is always an impedance value (Z) even if the circuit mode
     * was parallel.
     *
     * @throws IOException re-thrown from {@link Device#SendViaGPIB}
     *
     * @throws ScriptException re-thrown from {@link Device#getFloat}
     *
     * @return Returns an <code>MValue</code> object with the field <code>Valid</code>
     * set to <code>true</code> only if the Instrument's response was as expected
     * and with valid measurements. If <code>Valid</code> is <code>false</code> the
     * content of the other fields can be arbitrary. The returned value always contains
    ...
    private MValue Measure()
        ...
        if (m_UsedCommPort == CommPorts.GPIB) {
            ...
            String ans = QueryViaGPIB("EX");    // add \n?
     ...
     * decade. See {@link Device#GenerateValues(double, double, double, boolean, ArrayList, int)}
     * @param FreqLog When <code>false</code> the data points are equally spaced;
     ...
     * @throws DataFormatException when the Syntax Check failed.
     */
    // <editor-fold defaultstate="collapsed" desc="sweep F then V">
    @AutoGUIAnnotation(
        ...
        ParameterNames = {"Start Frequency [kHz] { [0.005, 13000] }", "Stop Frequency [kHz] { [0.005, 13000] }",
                          ...
                          "Bias Increment [V]", "logarithmic Bias spacing?", "Delay time [s]", "File Extension"},
        DefaultValues = {"1", "100", "1", "false",
                         "-0.5", "0.5", "1", "false",
                         "2", ".CV"},
        ToolTips = {"", "", "", "", "", "", "", "",
                    "<html>Time waited after applying the spot bias and<br>starting the frequency sweep.</html>", ""})
    @iC_Annotation(  MethodChecksSyntax = true )
    public void SweepFthenV(double FreqStart, double FreqStop, double FreqInc, boolean FreqLog,
        ...
        ArrayList<Double> V = GenerateValues(VStart, VStop, VInc, VLog, null, 4);

        // generate Frequency values
        ArrayList<Double> F = GenerateValues(FreqStart, FreqStop, FreqInc, FreqLog, null, 4);
        ...
        for (double v : V) {
            ...
            for (double f : F) {
                ...
                //Chart.AddXYDataPoint(SeriesNumber, DataPoint.Freq, DataPoint.Z);
                Chart.AddXYDataPoint(SeriesID_Z, DataPoint.Freq, DataPoint.Z);
                Chart.AddXYDataPoint(SeriesID_Phi, DataPoint.Freq, DataPoint.Phi);

                // Test third axis
                //Chart.AddXYDataPoint(SeriesID_Third, DataPoint.Freq, DataPoint.Freq);
        ...
        try {
            ...
            // write header line
            file.write("f [kHz?]\t|Z| [Ohm]\tphi [deg]\tVbias [V]\n");
            ...
            m_Logger.log(Level.SEVERE, "HP4192.SweepFthenV", ex);
