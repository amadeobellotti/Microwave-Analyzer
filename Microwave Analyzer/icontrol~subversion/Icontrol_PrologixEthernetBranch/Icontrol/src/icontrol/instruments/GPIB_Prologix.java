...
public class GPIB_Prologix extends GPIB_Driver {
    ...
    /** 
     * Defines if the native drivers or the Ethernet protocol should be used
     * to address the Prologix GPIB-USB or GPIB-Ethernet controller
     */
    private boolean m_UseEthernetProtocol;
...
//     };
     ...
     * 
     * @param UseEthernetProtocol It <code>true</code> the Ethernet protocol is
     * used to access the Prologix GPIB-Ethernet controller. If set to <code>false</code>
     * the native drivers for the GPIB-USB controller are used.
     *
    ...
    protected GPIB_Prologix(boolean UseEthernetProtocol)
           throws IOException {
        ...
        m_UseEthernetProtocol = UseEthernetProtocol;
    ...
    private void InitPrologix()
        ...
        IcontrolView.DisplayStatusMessage("First Attempt on ++ help read " + Integer.toString(Response.length()) + " bytes\n", false);
        ...
        if (Response.length() < 1700) {
            ...
            IcontrolView.DisplayStatusMessage("Second Attempt on ++ help read " + Integer.toString(Response.length()) + " bytes\n", false);
        }
...
//        WriteToPrologix("++eot_char 10");
     ...
     */
    // TODO delme after checking it's not used anymore
    ...
    protected long Send(String Msg)
        ...
        // do nothing if Msg is empty. See comment in GPIB_NI.Send
        if (Msg.isEmpty()) {
            return 0;
        }
     ...
     * went wrong. If an error occurred, it should be logged to the GPIB Logger.
     *
    ...
    private synchronized String ReadFromPrologix()
        ...
        if (false) {
            ...
            m_GPIB_Logger.log(Level.FINEST, "ReadFromPrologix: {0} bytes in RxQueue\n",
            ...
            while (AmountInRxQueue.getValue() > 0) {
                ...
                m_GPIB_Logger.log(Level.FINEST, "ReadFromPrologix just read {0} bytes\n",
    ...
    protected int ReadStatusByte()
        ...
        int Status;
        try {
            Status = Device.getInteger(StatusByte);
...
//        checkCommunicationError(str);
    ...
    protected void SpeedTestV2(int NrIterations) {
        ...
        // display status msg
        IcontrolView.DisplayStatusMessage("Starting GPIB_Prologix.SpeedTestV2().\n", false);
        ...
        for (int i=0; i<NrIterations; i++) {
            try {
                ...
                IcontrolView.DisplayStatusMessage(String.format("%3d (%4dms): %s\n", i, dT, idn), false);

            } catch (IOException ex) {
                // show and log error
                IcontrolView.DisplayStatusMessage("GPIB_Prologix.SpeedTestV2: IO failed.\n", false);
                return;
            }
        }

        // display status msg
        IcontrolView.DisplayStatusMessage("max dT = " + max_dT + "\n", false);
        IcontrolView.DisplayStatusMessage("The GPIB_Prologix.SpeedTestV2 is done.\n", false);
    }//</editor-fold>
}
